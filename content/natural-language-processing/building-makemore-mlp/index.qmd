...
#| fig-cap: "A plot for loss on different logarithm of learning rates"
g = torch.Generator().manual_seed(42)
C = torch.randn((27, 2), generator=g)
W1 = torch.randn((6, 100), generator=g)
b1 = torch.randn(100, generator=g)
...
plt.plot(lre, losses)
```
As @fig-loss-vs-lr shows, the loss bottoms out around an exponent of -1.0, which corresponds to a learning rate of $10^{-1}=0.1$, the value we had been using all along.
...
As @fig-loss-plot shows, the validation loss drops again right at step 10,000, exactly where the learning rate falls from 0.1 to 0.01.
...
plt.show()
```
As @fig-embedding shows, the vowels cluster together in the bottom-left corner, while `.`, the end-of-word token, sits far off in the top right.
